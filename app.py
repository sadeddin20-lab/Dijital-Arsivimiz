...
if uploaded_files:
    ...
    success_count = 0
    # Yükleme yapılırken ekranda bir yükleniyor animasyonu döner
    with st.spinner('Medya yükleniyor, lütfen bekleyin... / Pujant media, si us plau espereu...'):
        for uploaded_file in uploaded_files:
            ...
            # Sonra Drive'a gönder
            drive_id = upload_to_drive(local_path, file_name)
            if drive_id:
                success_count += 1
        
    # Tüm dosyalar bittikten sonra sonuç mesajı gösterilir
    if success_count == len(uploaded_files):
        st.success("🎉 Başarıyla yüklendi! / Tot s'ha penjat correctament!")
        # Misafirin başarı mesajını görebilmesi için ekranı yenilemeden önce 3 saniye bekletiyoruz
        time.sleep(3)
    else:
        st.warning(f"⚠️ {success_count}/{len(uploaded_files)} dosya yüklenebildi.")
        time.sleep(4)
        
    # Mesaj görüldükten sonra yükleyici kutusunu sıfırlayıp sayfayı temizliyoruz
...
# --- ANA SAYFA ---
# 🚨 İSİM BOYUTUNU 36PX YAPTIM, FOTOĞRAF ARTIK EKRANA TAM SIĞACAK 🚨
st.markdown("""
    <div style="text-align: center; font-size: 36px; font-weight: bold; color: #C5A034; margin-top: 20px; text-shadow: 2px 2px 8px #000;">
        𝓜𝓪𝓻𝓲𝓪 ∞ 𝓒𝓪𝓷𝓫𝓮𝓻𝓴
    </div>
